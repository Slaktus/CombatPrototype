...
public class AbstractGoTweenCollection : AbstractGoTween
{
    ...
    public override bool removeTweenProperty ( AbstractTweenProperty property )
    {
        for ( int i = 0 ; _tweenFlows.Count > i ; i++ )
        {
            ...
            if ( _tweenFlows[ i ].tween.removeTweenProperty( property ) )
                return true;
        }
    ...
    public override List<AbstractTweenProperty> allTweenProperties ()
    {
        ...
        for ( int i = 0 ; _tweenFlows.Count > i ; i++ )
        {
            ...
            propList.AddRange( _tweenFlows[ i ].tween.allTweenProperties() );
        }
    ...
    public override void reverse ()
    {
        ...
        for ( int i = 0 ; _tweenFlows.Count > i ; i++ )
        {
            ...
            if ( isReversed != _tweenFlows[ i ].tween.isReversed )
                _tweenFlows[ i ].tween.reverse();
            ...
            if ( isReversed || _isLoopingBackOnPingPong )
            {
                if ( _tweenFlows[ i ].startTime <= convertedElapsedTime )
                    _tweenFlows[ i ].tween.play();
            }
            else
            {
                if ( _tweenFlows[ i ].endTime >= convertedElapsedTime )
                    _tweenFlows[ i ].tween.play();
            }
        }
    ...
    #endregion AbstractTween overrides
}